...
$db = mysqli_connect('localhost','webchat','webchat','chat');
...
try {
    ...
    switch($action){
        case 'poll':
            $query = "SELECT * FROM chatlog WHERE data_created >= ?";
            $stmt = $db->prepare($query);
            $stmt -> bind_param('s',$lastPoll);
            $stmt -> execute();
            $stmt -> bind_result($id,$message,$session_id,$data_created);
        ...
        case 'send':
            ...
            $query = "INSERT INTO chatlog (message,sent_by,data_created) VALUES (?,?,?)";
            $stmt = $db->prepare($query);
